                    ...
                    <form method="post" action="{{ route('page.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="page_slug" id="page_slug" class="form-control"
                                value="{{ old('page_slug', $page->page_slug ?? '') }}" required>
                                @error('page_slug')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="page_content" id="summernote" rows="6" class="form-control summernote" required>{{ old('page_content', $page->page_content ?? '') }}</textarea>
                              @error('page_content')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" id="status" class="form-control" required>
                                <option value="Active" {{ old('status', $page->status ?? '') == 1 ? 'selected' : '' }}>
                                    Active</option>
                                <option value="Inactive" {{ old('status', $page->status ?? '') == 0 ? 'selected' : '' }}>
                                    Inactive</option>
                            </select>
                             @error('status')
                                <span class="text-danger">{{ $message }}</span>
